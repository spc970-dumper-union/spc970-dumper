...
static const char *get_region_name(u8 region) {
    switch (region) {
        ...
        case 0x04: return "Russia";
        case 0x05: return "China";
        case 0x06: return "Mexico";
        case 0x80: return "DEX / Test (DTL-H)";
        case 0x81: return "DEX / Test (DTL-H)";
        case 0x82: return "DEX / Test";
...
static void init_ps2_system(void) {
    ...
    // 4. Load PS2SDK Gamepad drivers (sio2man.irx + padman.irx embedded from PS2SDK!)
...
static void draw_progress_bar(int current, int total, const char *label) {
    ...
    for (int i = 0; i < bar_width; i++) {
        if (i < filled) scr_printf("=");
        else if (i == filled) scr_printf(">");
        else scr_printf(" ");
    }
...
// Initial hardware queries via pure SCMD verified against Ghidra MechaCon_Analysis.rep
static void query_initial_hardware(void) {
    ...
    log_printf("[INIT] SCMD 0x03-00: Region=0x%02X (%s), Major=%d, Minor=%02d, Rev=0x%02X (status: 0x%02X)\n",
               g_mecha_ver[0], get_region_name(g_mecha_ver[0]),
               g_mecha_ver[1], g_mecha_ver[2], g_mecha_ver[3], ver_stat);
    log_printf("[INIT] MechaCon Chip Part No: %s\n",
               get_mechacon_chip_desc(g_mecha_ver[1], g_mecha_ver[2], g_mecha_ver[0]));
    if (g_mecha_ver[0] == 0x80 || g_mecha_ver[0] == 0x81) {
        log_printf("[INIT] Note: Region 0x%02X indicates DEX / DTL-H Test Station (normal, not an error)\n", g_mecha_ver[0]);
    }
    ...
    u8 mn_stat = 0;
    if (mecha_read_model_name(g_cdvd_model, &mn_stat) == 0 && strlen(g_cdvd_model) > 0) {
        g_model_name_valid = 1;
        log_printf("[INIT] SCMD 0x17 Model Name: %s\n", g_cdvd_model);
    } else {
        g_model_name_valid = 0;
        strcpy(g_cdvd_model, "N/A (Early MechaCon)");
        log_printf("[INIT] SCMD 0x17 not present or returned error (stat: 0x%02X)\n", mn_stat);
    }
    ...
    if (mecha_read_rtc(g_mecha_rtc, &rtc_stat) == 0) {
        g_rtc_valid = 1;
        log_printf("[INIT] SCMD 0x08 RTC: %02X %02X %02X %02X %02X %02X %02X %02X (stat: 0x%02X)\n",
                   g_mecha_rtc[0], g_mecha_rtc[1], g_mecha_rtc[2], g_mecha_rtc[3],
                   g_mecha_rtc[4], g_mecha_rtc[5], g_mecha_rtc[6], g_mecha_rtc[7], rtc_stat);
    } else {
        g_rtc_valid = 0;
    ...
    if (mecha_read_console_id(g_console_id, &cid_stat) == 0) {
        ...
        log_printf("[INIT] SCMD 0x03-45 Console ID: %02X %02X %02X %02X %02X %02X %02X %02X\n",
        ...
        log_printf("[INIT] SCMD 0x03-45 Console ID not loaded / unsupported (stat: 0x%02X)\n", cid_stat);
    }
    ...
    if (mecha_read_ilink_id(g_ilink_id, &ilink_stat) == 0) {
        g_ilink_id_valid = 1;
        log_printf("[INIT] SCMD 0x12 i.Link ID: %02X %02X %02X %02X %02X %02X %02X %02X\n",
                   g_ilink_id[0], g_ilink_id[1], g_ilink_id[2], g_ilink_id[3],
                   g_ilink_id[4], g_ilink_id[5], g_ilink_id[6], g_ilink_id[7]);
    } else {
        g_ilink_id_valid = 0;
        log_printf("[INIT] SCMD 0x12 i.Link ID not loaded / unsupported (stat: 0x%02X)\n", ilink_stat);
    }

    // 8. Early NVRAM Header Query: Read Serial & Model ID via SCMD 0x0A so folder name is consistent from boot
    u8 dummy_nvm[512];
    memset(dummy_nvm, 0xFF, sizeof(dummy_nvm));
    u16 w = 0; u8 st = 0;
    // Old layout words: 0x0E4 (Model), 0x0E6..0x0E7 (Serial)
    if (mecha_read_nvm_word(0x0E4, &w, &st) == 0) { dummy_nvm[0x1C8] = (u8)(w >> 8); dummy_nvm[0x1C9] = (u8)w; }
    if (mecha_read_nvm_word(0x0E6, &w, &st) == 0) { dummy_nvm[0x1CC] = (u8)(w >> 8); dummy_nvm[0x1CD] = (u8)w; }
    if (mecha_read_nvm_word(0x0E7, &w, &st) == 0) { dummy_nvm[0x1CE] = (u8)(w >> 8); dummy_nvm[0x1CF] = (u8)w; }
    // New layout words: 0x0F8 (Model), 0x0FA..0x0FB (Serial)
    if (mecha_read_nvm_word(0x0F8, &w, &st) == 0) { dummy_nvm[0x1F0] = (u8)(w >> 8); dummy_nvm[0x1F1] = (u8)w; }
    if (mecha_read_nvm_word(0x0FA, &w, &st) == 0) { dummy_nvm[0x1F4] = (u8)(w >> 8); dummy_nvm[0x1F5] = (u8)w; }
    if (mecha_read_nvm_word(0x0FB, &w, &st) == 0) { dummy_nvm[0x1F6] = (u8)(w >> 8); dummy_nvm[0x1F7] = (u8)w; }
    g_model_id = extract_model_id_from_nvram(dummy_nvm);
    g_serial = extract_serial_from_nvram(dummy_nvm, &g_emcs);
    if (g_serial > 0) {
        log_printf("[INIT] SCMD 0x0A Early Serial: %07u, EMCS: 0x%02X, Model ID: 0x%04X (%s)\n",
...
// Compute dynamic destination folder, prioritizing host: (for PCSX2 / emulators) then mass: (for real PS2)
static void update_dump_directory(void) {
    ...
    if (g_serial > 0) {
        snprintf(folder_name, sizeof(folder_name), "MECHA_v%d.%02d_%07u", major, minor, g_serial);
    } else if (g_model_name_valid && strlen(g_cdvd_model) > 0 && strncmp(g_cdvd_model, "N/A", 3) != 0) {
        snprintf(folder_name, sizeof(folder_name), "MECHA_v%d.%02d_%s", major, minor, g_cdvd_model);
    } else if (g_model_id != 0) {
        snprintf(folder_name, sizeof(folder_name), "MECHA_v%d.%02d_ID%04X", major, minor, g_model_id);
    } else {
        snprintf(folder_name, sizeof(folder_name), "MECHA_v%d.%02d_DUMP", major, minor);
    }

    g_storage_ready = 0;

    // 1. Priority 1: Check host: (native PC filesystem on PCSX2 / Play! / test environment)
    ...
    // 3. If neither host: nor mass: is accessible yet (e.g. real PS2 without USB stick inserted)
    snprintf(g_dump_dir, sizeof(g_dump_dir), "mass:/%s", folder_name);
    g_storage_ready = 0;
    log_printf("[DIR] No ready storage device detected yet. Default: %s (insert USB drive)\n", g_dump_dir);
}
...
static void print_hex_block(u16 base_addr, const u8 *data, int len) {
    for (int i = 0; i < len; i += 16) {
        ...
        for (int j = 0; j < 16; j++) {
            if (i + j < len) scr_printf("%02X ", data[i + j]);
            else scr_printf("   ");
        }
        scr_printf("|");
        for (int j = 0; j < 16; j++) {
...
static void show_system_info(void) {
    ...
    if (g_console_id_valid) {
        scr_printf(" [SCMD 0x03-45] CID   : %02X %02X %02X %02X %02X %02X %02X %02X\n",
                   g_console_id[0], g_console_id[1], g_console_id[2], g_console_id[3],
                   g_console_id[4], g_console_id[5], g_console_id[6], g_console_id[7]);
    } else {
    ...
    if (g_ilink_id_valid) {
        scr_printf(" [SCMD 0x12] i.Link   : %02X %02X %02X %02X %02X %02X %02X %02X\n\n",
                   g_ilink_id[0], g_ilink_id[1], g_ilink_id[2], g_ilink_id[3],
                   g_ilink_id[4], g_ilink_id[5], g_ilink_id[6], g_ilink_id[7]);
    } else {
...
static void backup_nvram_action(void) {
    ...
    if (read_errs > 0) {
        log_printf("[WARN] NVRAM backup completed with %d word read errors (status != 0x00)\n", read_errs);
    }
    ...
    log_printf("[NVRAM] Decoded Serial: %07u, EMCS: 0x%02X, Model ID: 0x%04X (%s)\n",
               g_serial, g_emcs, g_model_id, get_model_id_desc(g_model_id));
    ...
    scr_printf("    Decoded Serial: %07u | Model ID: 0x%04X (%s)\n",
               g_serial, g_model_id, get_model_id_desc(g_model_id));
    if (saved_mass) {
...
static void probe_config_overflow_action(void) {
    ...
    scr_printf("     SCMD 0x40 (read mode 0, region 2, 16 blocks) -> 16x SCMD 0x41\n");
    ...
    if (read_errs < 0) {
        scr_printf(" [-] Failed to open config region for reading (stat: 0x%02X)\n\n", probe_stat);
        ...
        for (int i = 112; i < 256; i++) {
            if (probe256[i] != 0) non_zero_overflow++;
        }

        if (non_zero_overflow == 0) {
            scr_printf(" [!] BLOCKS 8-15 ARE ALL ZEROES (Signature of CXP101064 / 10K!)\n");
            scr_printf("     EEPROM worker fields are NOT at offset +0xB0 in this chip.\n");
            log_printf("[PROBE] Diagnosis: Blocks 8-15 all zero (CXP101064 / MD 1.36 layout)\n");
        } else {
            scr_printf(" [+] Active RAM structures detected in overflow region (+%d bytes)!\n", non_zero_overflow);
            scr_printf("     Matches CXP102064 / CXP103049 layout signature.\n");
            log_printf("[PROBE] Diagnosis: Active RAM structures found in overflow area\n");
        ...
        if (ft) {
            ...
            fprintf(ft, "raw_scmd_03_00=%02X %02X %02X %02X\n",
                    g_mecha_ver[0], g_mecha_ver[1], g_mecha_ver[2], g_mecha_ver[3]);
    ...
    // Also take an extended 512-byte snapshot (32 blocks) to find where CXP101064 worker is
    ...
    log_printf("[PROBE] SCMD 0x40 open (write, reg 2, count 0): ret=%d, stat=0x%02X\n", ret, open_stat);
    ...
    if (ret == 0) {
        ...
        if (wr_stat == 0x00) {
            scr_printf(" [+] OVERFLOW WRITES ACCEPTED! Hardware has no bounds check.\n");
            log_printf("[PROBE] Overflow writes accepted (no bounds check)\n");
        } else {
            scr_printf(" [-] OVERFLOW WRITE REJECTED! Hardware returned error 0x%02X.\n", wr_stat);
...
// Menu 4: Full Hardware & RAM Mapping (All Config Regions 0..7, PMAP Subcommands, SCMD Sweep)
static void full_hardware_mapping_action(void) {
    scr_clear();
    scr_printf("=====================================================\n");
    scr_printf("   Full MechaCon Hardware & RAM Mapping (Regions 0-7)\n");
    scr_printf("=====================================================\n\n");

    log_printf("[MAP] Starting comprehensive hardware and RAM mapping...\n");
    update_dump_directory();
    ...
    for (int r = 0; r < 8; r++) {
        ...
        // Underflow read: req_count = 0, read 16 blocks (256 bytes)
        u8 buf256[256];
        memset(buf256, 0, sizeof(buf256));
        u8 st_uf = 0xFF;
        int err_uf = mecha_read_ram_probe_blocks((u8)r, 0, 16, buf256, &st_uf);
        if (err_uf >= 0 && st_uf == 0x00) {
            reg_results[r].supported_uf = 1;
            reg_results[r].stat_uf = st_uf;
            for (int i = 0; i < 112; i++) {
                if (buf256[i] != 0) reg_results[r].non_zero_low++;
            }
            for (int i = 112; i < 256; i++) {
                if (buf256[i] != 0) reg_results[r].non_zero_overflow++;
            }
            for (int i = 0; i <= 256 - 4; i++) {
                u32 val = buf256[i] | (buf256[i+1] << 8) | (buf256[i+2] << 16);
                if ((val >= 0xFC0000 && val <= 0xFFFFFF) || (val >= 0xFD0000 && val <= 0xFFFFFF)) {
        ...
        scr_printf("  Region %d: std=%s (stat 0x%02X) | underflow=%s [NZ low:%d, over:%d, ptrs:%d]\n",
                   ...
                   reg_results[r].potential_rom_ptrs);
        log_printf("[MAP] Region %d: std=%d (stat=0x%02X), underflow=%d (stat=0x%02X), low_nz=%d, over_nz=%d, ptrs=%d\n",
                   r, reg_results[r].supported_std, st_std, reg_results[r].supported_uf, st_uf,
                   reg_results[r].non_zero_low, reg_results[r].non_zero_overflow, reg_results[r].potential_rom_ptrs);
    }

    // 3. Scan PMAP Subcommands (SCMD 0x03, 0x00..0x4F)
    scr_printf("\n [*] Step 3/4: Sweeping SCMD 0x03 PMAP Subcommands (0x00..0x4F)...\n");
    char pmap_path[256];
    snprintf(pmap_path, sizeof(pmap_path), "%s/PMAP_SUBCMDS.TXT", g_dump_dir);
    FILE *fp_pmap = fopen(pmap_path, "w");
    if (fp_pmap) {
        fprintf(fp_pmap, "=== MechaCon SCMD 0x03 PMAP Subcommands Sweep ===\n");
        fprintf(fp_pmap, "MechaCon: v%d.%02d (Reg 0x%02X, Rev 0x%02X) | Chip: %s\n\n",
                g_mecha_ver[1], g_mecha_ver[2], g_mecha_ver[0], g_mecha_ver[3],
                get_mechacon_chip_desc(g_mecha_ver[1], g_mecha_ver[2], g_mecha_ver[0]));
    }

    int valid_pmap = 0;
    for (int sub = 0; sub <= 0x4F; sub++) {
        u8 out16[16] = { 0 };
        u8 st = 0xFF;
        int r = mecha_query_scmd03_subcmd((u8)sub, out16, &st);
        if (r == 0 && out16[0] != 0x80) {
            valid_pmap++;
            log_printf("[MAP_PMAP] Subcmd 0x%02X: stat=0x%02X | %02X %02X %02X %02X %02X %02X %02X %02X\n",
                       sub, out16[0], out16[1], out16[2], out16[3], out16[4], out16[5], out16[6], out16[7], out16[8]);
        }
        if (fp_pmap) {
            fprintf(fp_pmap, "Subcmd 0x%02X: stat=0x%02X [ret=%d] | ", sub, out16[0], r);
            for (int k = 0; k < 16; k++) {
                fprintf(fp_pmap, "%02X ", out16[k]);
            }
            fprintf(fp_pmap, "\n");
        }
    }
    if (fp_pmap) fclose(fp_pmap);
    scr_printf("  PMAP Sweep complete: %d active/responding subcommands found.\n", valid_pmap);
    ...
    if (fr) {
        ...
        fprintf(fr, "Region | Std 4-Blk | Underflow 16-Blk | Low Non-Zero | Overflow Non-Zero | ROM Pointers\n");
        fprintf(fr, "-------+-----------+------------------+--------------+-------------------+-------------\n");
        for (int r = 0; r < 8; r++) {
        ...
        if (best_region >= 0 && max_over > 0) {
            ...
            fprintf(fr, "[!] No standard overflow structures detected in any region 0-7.\n");
    ...
    scr_printf("\n[+] Full mapping complete! All files saved to:\n    %s/\n", g_dump_dir);
    wait_for_cross();
}
...
static void full_dump_and_verify_action(void) {
    ...
    scr_printf("     Validating pre-flight staging & dumping %u chunks (%s)...\n",
               expected_chunks, is_v3_device ? "192 KB, Banks FD-FF" : "256 KB, Banks FC-FF");
    log_printf("[AUTO_DUMP] Starting SPC970 ROM dump exploit (%s)...\n",
               is_v3_device ? "v3 192KB" : "v2 256KB");
    u32 dumped_rom_size = 0;
    int dump_ret = mecha_dump_full_rom(g_rom_buffer, &dumped_rom_size, g_nvram_backup, draw_progress_bar);
    log_printf("[AUTO_DUMP] ROM dump returned: %d (dumped %u bytes)\n", dump_ret, dumped_rom_size);
    ...
    if (dump_ret < 0) {
        ...
        log_printf("[AUTO_RESTORE] Exploit failed (%d), performing guaranteed NVRAM restore...\n", dump_ret);
        ...
        scr_setXY(2, 16);
        scr_printf(" [+] NVRAM Auto-Restored (%d write errors). Safe to reboot!\n\n", rest_err);

        if (dump_ret == -1) {
            scr_printf(" [-] SCMD 0x40/0x42 configuration open/write rejected!\n");
            scr_printf("     This MechaCon chip rejected write mode or block count 0.\n");
        } else if (dump_ret == -2) {
    ...
    if (res.all_passed) {
        scr_printf("\n[+] SUCCESS: 100%% INTACT BIT-PERFECT DUMP! ALL POST CHECKS PASSED!\n");
    } else {
    ...
    if (f_info) {
        ...
        fprintf(f_info, "ROM Dump Size         : %u KB (%u bytes, %s)\n",
                ...
                (dumped_rom_size == 196608) ? "v3 Native Banks FD-FF" : "v2 Full Banks FC-FF");
        fprintf(f_info, "Model Name (SCMD 0x17): %s\n", g_cdvd_model);
        fprintf(f_info, "MechaCon Chip Part No : %s\n", get_mechacon_chip_desc(g_mecha_ver[1], g_mecha_ver[2], g_mecha_ver[0]));
        fprintf(f_info, "Model ID (NVRAM)      : 0x%04X (%s)\n", g_model_id, get_model_id_desc(g_model_id));
        fprintf(f_info, "Serial Number (NVRAM) : %07u (EMCS: 0x%02X)\n", g_serial, g_emcs);
        fprintf(f_info, "BIOS ROMVER           : %s\n", g_romver);
        fprintf(f_info, "MechaCon Ver (0x03-00): MD 1.39 v%d.%02d (Region: 0x%02X [%s%s], Rev 0x%02X)\n",
                g_mecha_ver[1], g_mecha_ver[2], g_mecha_ver[0], get_region_name(g_mecha_ver[0]),
                (g_mecha_ver[0] == 0x80 || g_mecha_ver[0] == 0x81) ? " / DEX" : "",
                g_mecha_ver[3]);
        fprintf(f_info, "DSP Version  (0x03-01): 0x%02X\n", g_dsp_ver);
        if (g_rtc_valid) {
            fprintf(f_info, "Hardware RTC (0x08)   : %02X %02X %02X %02X %02X %02X %02X\n",
                    g_mecha_rtc[1], g_mecha_rtc[2], g_mecha_rtc[3],
                    g_mecha_rtc[4], g_mecha_rtc[5], g_mecha_rtc[6], g_mecha_rtc[7]);
        } else {
        ...
        if (g_console_id_valid) {
            fprintf(f_info, "Console ID   (0x03-45): %02X%02X%02X%02X%02X%02X%02X%02X\n",
                    g_console_id[0], g_console_id[1], g_console_id[2], g_console_id[3],
                    g_console_id[4], g_console_id[5], g_console_id[6], g_console_id[7]);
        } else {
            fprintf(f_info, "Console ID   (0x03-45): N/A (Not loaded / early MechaCon)\n");
        }
        if (g_ilink_id_valid) {
            fprintf(f_info, "i.Link ID    (0x12)   : %02X%02X%02X%02X%02X%02X%02X%02X\n",
                    g_ilink_id[0], g_ilink_id[1], g_ilink_id[2], g_ilink_id[3],
                    g_ilink_id[4], g_ilink_id[5], g_ilink_id[6], g_ilink_id[7]);
        } else {
            fprintf(f_info, "i.Link ID    (0x12)   : N/A (Not loaded / early MechaCon)\n");
        }
        fprintf(f_info, "\nHardware Checksums (POST Routine 0xFF5170):\n");
        fprintf(f_info, "Mirror Match   : %s\n", res.mirror_match ? "PASS" : "FAIL");
        for (int i = 0; i < res.total_sections; i++) {
            fprintf(f_info, "Section %d (%s): %s [Computed: 0x%04X, Expected: 0x%04X]\n",
                    i + 1, res.sections[i].name,
                    res.sections[i].passed ? "PASS" : "FAIL",
                    res.sections[i].computed_checksum, res.sections[i].expected_checksum);
        }
...
int main(int argc, char *argv[]) {
    ...
    for (int i = 1; i < argc; i++) {
        if (!argv[i]) continue;
        if (strcmp(argv[i], "--auto") == 0 || strcmp(argv[i], "--dump") == 0) auto_action = 4;
        else if (strcmp(argv[i], "--probe") == 0) auto_action = 3;
        else if (strcmp(argv[i], "--backup") == 0) auto_action = 2;
        else if (strcmp(argv[i], "--info") == 0) auto_action = 1;
        else if (strcmp(argv[i], "--map") == 0) auto_action = 5;
    }
    ...
    while (1) {
        ...
        scr_printf("    PS2 MechaCon Diagnostic & CXP103049 Dumper Tool  \n");
        scr_printf("=====================================================\n");
        scr_printf(" Chip: %-16s | Ver: v%d.%02d (Reg 0x%02X)\n\n",
                   get_mechacon_chip_desc(g_mecha_ver[1], g_mecha_ver[2], g_mecha_ver[0]),
        ...
        scr_printf(" %s [4] Full Hardware & RAM Mapping (All Regions & PMAP)\n", (selected == 3) ? "->" : "  ");
